            ...
            <LayerGroup>
              {huntingCoords && huntingCoords.map((coords, idx) => {
                return coords.geometry.rings && coords.geometry.rings.map((item, idx2) => {
                  return <Polygon color={"purple"} weight={2} smoothFactor={2.0} positions={item.map(point => [point[1],point[0]])} key={idx+idx2}/>
                })
              })};
            </LayerGroup>
          </LayersControl.Overlay>
          <LayersControl.Overlay name="Public Land">
            <LayerGroup>
              {publicCoords && publicCoords.map((coords, idx) => {
                return coords.geometry.rings && coords.geometry.rings.map((item, idx2) => {
                  return <Polygon color={"blue"} weight={2} smoothFactor={2.0} positions={item.map(point => [point[1],point[0]])} key={idx+idx2} />
                });
              })};
            </LayerGroup>
          </LayersControl.Overlay>
            ...
            <LayerGroup>
              {tracks && tracks.map((coords, idx) => {
                return coords.geometry.paths && coords.geometry.paths.map((item, idx2) => {
                  return <Polyline color={"red"} weight={2} smoothFactor={2.0} positions={item.map(point => [point[1],point[0]])} key={idx+idx2} />
                });
              })};
            </LayerGroup>
          </LayersControl.Overlay>
          <LayersControl.Overlay name="Huts">
            <LayerGroup>
              {huts && huts.map((coords, idx) => {
                return <CircleMarker color={"orange"} center={[coords.geometry.y, coords.geometry.x]} key={idx} />
              })};
            </LayerGroup>
          </LayersControl.Overlay>
        </LayersControl>
